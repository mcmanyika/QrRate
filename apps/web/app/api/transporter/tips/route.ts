...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get total earnings separately (all tips, not just current page)
    const { data: allTips } = await supabase
      .from('tip')
      .select('operator_amount_cents')
      .in('vehicle_id', vehicleIds)
      .eq('stripe_status', 'succeeded')

    const totalEarnings = allTips?.reduce(
      (sum, tip) => sum + (tip.operator_amount_cents || 0),
      0
    ) || 0
