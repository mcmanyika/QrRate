...
export async function POST(request: NextRequest) {
  try {
    ...
    const { business_id, qr_code_id, stars, tags, comment, photo_urls, device_hash } = body

    if (!business_id || !stars || !device_hash) {
      return NextResponse.json(
        { error: 'Missing required fields: business_id, stars, device_hash' },
    ...
    if (qr_code_id) {
      ...
      if (qrError || !qrCode || qrCode.business_id !== business_id) {
        return NextResponse.json(
